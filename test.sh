...
docker run -d \
    ...
    -e EXTERNAL_BACKEND_HOSTNAME=httpbin.org \
    -e EXTERNAL_BACKEND_PORT=443 \
    -e EXTERNAL_HEADER_UP_HOST=httpbin.org \
    -e EXTERNAL_BACKEND_PATTERN="^/basic-auth/.*" \
    -e EXTERNAL_AUTH_USERNAME=user \
    -e EXTERNAL_AUTH_PASSWORD=password \
    caddy-proxy
...
echo ""
echo "Test the external backend (httpbin.org):"
echo "curl http://localhost:8080/basic-auth/user/password"
echo ""
